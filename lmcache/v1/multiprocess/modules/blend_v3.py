...
class BlendTokenRangeMatcherV3:
    ...
    def _content_is_live(self, poly_hash: int) -> bool:
        ...
        Chained token hashes differ per preamble, so only a content check
        stops a blend write-back from displacing the warm incumbent.
    ...
    def on_new_token_hashes(
        ...
        with self._lock:
            # Skip windows fully covered by already-registered content: they add no
            # new coverage, and their store-side aux encode was skipped (poisoned
            # page), so indexing them would shadow the aux-carrying originals.
...
class _CBReadGroups:
    ...
    The two legs consume different planes, so each keys/locks its own set:

    Attributes:
        gids: The BLEND leg's set, ascending: attention plus the standalone
            fused-aux group when present, never recurrent-state groups.
        prefix_gids: The PREFIX leg's set, ascending: every recurrent-state
            group plus the attention group, never aux.
        recurrent_gids: The recurrent-state groups alone, ascending.
